...
const categoryDefaults = {
  ...
  'ארוחות': { icon: '🍖', color: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)' },
  'משקאות': { icon: '🥤', color: 'linear-gradient(135deg, #30cfd0 0%, #330867 100%)' },
  ...
  'סלטים': { icon: '🥗', color: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
  'מבצעים': { icon: '🎁', color: 'linear-gradient(135deg, #ff9a9e 0%, #fecfef 100%)' },
...
export default function ProductManagementPage() {
  ...
  const categoriesList = ['הכל', 'ארוחות', 'משקאות', 'קינוחים', 'תוספות', 'סלטים', 'מנות ראשונות', 'מבצעים']
  ...
  const createCategoriesTable = async () => {
    const defaultCategories = [
      ...
      { name: 'ארוחות', description: 'מנות עיקריות', display_order: 2, is_active: true },
      { name: 'קינוחים', description: 'קינוחים ומתוקים', display_order: 3, is_active: true },
      { name: 'משקאות', description: 'משקאות חמים וקרים', display_order: 4, is_active: true }
    ]
  ...
  async function handleGalleryUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const newGalleryImages = [...productFormData.gallery_images, ...uploadedUrls]
      setProductFormData({ ...productFormData, gallery_images: newGalleryImages })
  ...
  function removeGalleryImage(index: number) {
    const newGalleryImages = productFormData.gallery_images.filter((_, i) => i !== index)
  ...
  async function handleProductSubmit(e: React.FormEvent) {
    ...
    try {
      const productData = {
        ...
        price: productFormData.price ? parseFloat(productFormData.price) : parseFloat(productFormData.base_price) || 0,
        description: productFormData.description || null,
        image_url: productFormData.image_url || null,
        gallery_images: productFormData.gallery_images.length > 0 ? productFormData.gallery_images : null,
  ...
  async function handleProductDelete(id: string, productName: string) {
  ...
  const filteredProductsForMove = products.filter(p => 
                  ...
                  <div style={{
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setProductFormData({...productFormData, base_price: e.target.value})}
                        style={{
                          width: '100%',
                          padding: '10px 15px',
                          border: '1px solid #ddd',
